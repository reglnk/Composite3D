...
namespace cm3d::Utility::GL::Preprocessor
{
	int deleteComments(const char *srcbeg, const char *srcend, DynArray<char> *output)
	{
		for (const char *findCh = srcbeg; findCh != srcend; )
		{
			...
			const size_t curSize = output->length();
			const size_t blkSize = 1 + cmt - findCh;
			output->resize(curSize + blkSize);
			memcpy(output->begin() + curSize, findCh, blkSize);
	...
	int processDirective (
		...
		if (type == Directive::Pragma)
		{
			...
			{
				// @todo fix: that shit cuts code off instead of including the file
